...
function setup()
{
    ...
    agentFour = new Circle(width - 100, height - 100, 20, 5, colorTwo, controllerTwo);
...
function draw()
{
    ...
    /* Collision with goals - Rule variant ONE

    if (agentOne.overlaps(teamTwoGoal) || agentTwo.overlaps(teamTwoGoal) || agentThree.overlaps(teamOneGoal) || agentFour.overlaps(teamOneGoal))
    {
        agents.forEach((agent) =>
        {
            return agent.reset();
    ...
    */
    ...
    agents.forEach(agent =>
    {
        if (agent == agentOne || agent == agentTwo)
        {
            if (agent.overlaps(teamTwo))
            {
                ...
                teamOne.score++;
                teamTwo.radius = teamTwo.baseGoalRadius * (maxScore - teamOne.score) / maxScore;
            }
        }
        else
        {
            if (agent.overlaps(teamOne))
            {
                ...
                teamTwo.score++;
                teamOne.radius = teamOne.baseGoalRadius * (maxScore - teamTwo.score) / maxScore;
            }
        }
    });
    ...
    agents.forEach(agent =>
    {
        if (agent == agentOne || agent == agentTwo)
        {
            let overlapsWithThree = (agent.overlaps(agentThree) && agentThree.currentSpeed <= agentThree.snapSpeed);
            let overlapsWithFour = (agent.overlaps(agentFour) && agentFour.currentSpeed <= agentFour.snapSpeed);
        ...
        {
            let overlapsWithOne = (agent.overlaps(agentOne) && agentOne.currentSpeed <= agentOne.snapSpeed);
            let overlapsWithTwo = (agent.overlaps(agentTwo) && agentTwo.currentSpeed <= agentTwo.snapSpeed);
